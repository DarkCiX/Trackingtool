...
public class Onduleur {
	public Onduleur(int id, String date_intervention, String debut_intervention,String etat,String fin_intervention, String client,
	...
	public static void save(String entity,Onduleur ond) throws Exception {
	  	 MongoClient conex=DbHelper.getConnexionMG();
		 MongoDatabase database = DbHelper.getDB(conex);
	  	try{
		  	MongoCollection<Onduleur> dbCollection =  database.getCollection(entity,Onduleur.class);
//			int id=(int)dbCollection.countDocuments();
			int id = Onduleur.getMaxId("Onduleur");
			int newId = id +1;
			ond.setId(newId);
		  	dbCollection.insertOne(ond);
		  	conex.close();
	  	}catch(Exception ex) {
	  		System.out.println(ex.getMessage());
	  	}
  }
	 public static ArrayList<Onduleur> findAll(String entity) throws Exception{
		  MongoClient conex=DbHelper.getConnexionMG();
		 MongoDatabase database = DbHelper.getDB(conex);
		 MongoCollection<Onduleur> dbCollection =  database.getCollection(entity,Onduleur.class);
		 FindIterable<Onduleur> array =  dbCollection.find();
		 MongoCursor<Onduleur> cursor = array.iterator();
		 ArrayList<Onduleur> rep = new ArrayList<Onduleur>();
		 int count =0;
		 while(cursor.hasNext()) {
			 Onduleur document = cursor.next();
		     rep.add(document);
		     count++;
		 }
		 conex.close();
	 ...
	 public static void updateRow(String entity,Onduleur autre) throws Exception {
		  MongoClient conex=DbHelper.getConnexionMG();
		 MongoDatabase database = DbHelper.getDB(conex);
		 try {
	 ...
	 public static int getMaxId(String entity) throws Exception {
		 ...
		 obj.put("_id",-1);
		 FindIterable<Onduleur> array = dbCollection.find().sort(obj).limit(1);
		 MongoCursor<Onduleur> cursor = array.iterator();
		 Onduleur document = new Onduleur();
		 if (cursor.hasNext()) {
	 ...
	 public static Onduleur getdetail_Intervention_Onduleur(String entity,String ids) throws Exception {
		  MongoClient conex=DbHelper.getConnexionMG();
		 MongoDatabase database = DbHelper.getDB(conex);
		 MongoCollection<Onduleur> dbCollection =  database.getCollection(entity,Onduleur.class);
		 BasicDBObject obj = new BasicDBObject();
		 ...
		 obj.append("_id", id);   
		 FindIterable<Onduleur> array =  dbCollection.find(obj);
		 MongoCursor<Onduleur> cursor = array.iterator();
		 Onduleur document = new Onduleur();
		 if (cursor.hasNext()) {
			 document = cursor.next();
